...
func doPrepareHeader(genParams *generateParams, chain *core.BlockChain, config *Config, chainConfig *params.ChainConfig, extra []byte, engine consensus.Engine) (*types.Header, *types.Header, error) {
	...
	// Set the extra field.
	if len(extra) != 0 && chainConfig.Optimism == nil { // Optimism chains must not set any extra data.
		header.Extra = extra
	}
...
func (w *worker) generateWork(genParams *generateParams) (*types.Block, *big.Int, error) {
	...
	finalizeFn := func(env *environment, orderCloseTime time.Time,
		...
		if genParams.onBlock != nil {
			go genParams.onBlock(block, profit, orderCloseTime, blockBundles, allBundles, usedSbundles)
		}
